...
#
#  [*default_log_levels*]
#    (optional) List of comma-separated logger=level pairs.
#    Defaults to undef.
#    Example: 'amqp=WARN,amqplib=WARN,boto=WARN,qpid=WARN,sqlalchemy=WARN,suds=INFO,iso8601=WARN,\
...
class ceilometer::logging(
  ...
  if $log_date_format {
    ceilometer_config {
      'DEFAULT/log_date_format' : value => $instance_format;
      }
    }
  else {
    ceilometer_config {
